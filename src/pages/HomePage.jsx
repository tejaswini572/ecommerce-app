    ...
    <div className="bg-yellow-100 dark:bg-gray-900 min-h-screen p-8">
      ...
      <div className="flex gap-2 mb-6 justify-center flex-wrap font-serif">
        <button onClick={() => setSelectCategory('all')}
          className="bg-yellow-200 px-4 py-2 rounded-full shadow hover:bg-yellow-300 dark:bg-gray-800 dark:text-white">All</button>
        {categories.map(cat => (
          <button key={cat} onClick={() => setSelectCategory(cat)}
            className="bg-yellow-200 px-4 py-2 rounded-full shadow hover:bg-yellow-300 dark:bg-gray-800 dark:text-white">{cat}</button>
        ))}
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {currentProducts.map(product => (
          <div key={product.id} onClick={() => navigate(`/product/${product.id}`)}
            ...
            <h2 className="font-bold text-sm line-clamp-2 mt-2">{product.title}</h2>
            <p className="text-green-600">${product.price}</p>
            <p className="font-bold text-sm">Category: {product.category}</p>
            <p className="font-bold text-sm">Rating: {product.rating.rate}</p>
            <button onClick={(e) => {
              e.stopPropagation()
              isWishlisted(product.id) ? dispatch(removeFromWishlist(product.id)) : dispatch(addToWishlist(product))
      ...
      <div className="flex justify-center items-center gap-4 mt-6">
        <button className="px-4 py-4 bg-blue-500 text-white rounded disabled:bg-gray-300"
          disabled={currentPage === 1} onClick={() => setCurrentPage(currentPage - 1)}>Prev</button>
        <button className="px-4 py-4 bg-blue-500 rounded text-white disabled:bg-gray-300"
          disabled={currentPage === totalPages} onClick={() => setCurrentPage(currentPage + 1)}>Next</button>
      </div>
    </div>
  )
}
